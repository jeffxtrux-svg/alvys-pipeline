...
**What it is.** Triumph went live week of 6/23/26. X-Trux (asset side) flows correctly. **X-Linx (brokerage) side broke at go-live** — Alvys was not passing carrier payables to Triumph alongside AR invoices. 144 invoices from the initial buyout were unrecorded in both Alvys and QBO. Meeting 6/26/26: Alvys acknowledged the gap and owns the fix. **Update 6/30:** Cade Vanneman (Alvys) confirmed TriumphPay → QBO integration releasing **Wednesday 7/2** — testing wraps up Tuesday 7/1. **Manual mitigation in place:** `src/triumph_converter.py` (local script, not yet committed) bridges the gap in the meantime — joins an Alvys Factoring Batch export against Triumph's BatchTransactionDetail funded-invoice report and produces the CSV Alvys needs for "Upload Payment Report," so funded invoices keep getting recorded in Alvys even though the native integration isn't live yet. **Watch:** confirm Wednesday 7/2 release ships and 144-invoice sync completes; X-Linx carrier payables flowing to Triumph; AR aging shortening. Once the native integration is live, the manual script can be retired (or kept as a reconciliation check). Close this risk once integration confirmed live. **Owner:** JB / Jeff. See [[Factoring]], [[Financial Performance]], [[Decision Journal]], [[Playbook — Factoring Partner Switch]].
