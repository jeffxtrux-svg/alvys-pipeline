...
- **2026-06-26 — Alvys integration meeting.** Alvys owns the fix entirely; no custom build from XFreight. Deliverables: sync 144 invoices to QBO; QBO third-party accounting setup; Triumph Audit feature turned off for XFreight. Target architecture: Triumph → Alvys (two-way) → QBO. See [[Factoring]], [[Decision Journal]].
- **2026-06-30 — Alvys confirms Wednesday 7/2 release.** Cade Vanneman (Alvys) email: TriumphPay → QBO integration releasing Wednesday 7/2; testing wraps up Tuesday 7/1. **Watch:** confirm release ships and 144-invoice sync completes by 7/3; close this risk entry once integration confirmed live.
- **2026-06-30 — Manual bridge script built (`src/triumph_converter.py`).** While waiting on the native Alvys↔Triumph integration, a local script joins an Alvys Factoring Batch export against Triumph's BatchTransactionDetail funded-invoice report and produces the CSV Alvys needs for Upload Payment Report. Keeps funded invoices flowing into Alvys manually in the interim. Not yet committed to git as of this entry — recommend committing once verified working in production use.
